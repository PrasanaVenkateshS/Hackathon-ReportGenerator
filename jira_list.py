...
from agent_roles import (RS_Agent_Name, RS_Agent_Role, BA_Agent_Name, BA_Agent_Role,
                         ...
                         planning_agent_Name, planning_agent_Role_jiralist)
...
CONTEXT_SEARCH_FILTER_PATH = os.getenv("AZURE_BLOB_JIRALIST_PATH").split(",")
...
planning_agent = AssistantAgent(planning_agent_Name,
    ...
    system_message=planning_agent_Role_jiralist
)
...
async def main():
    user_prompt = """
Identify comprehensive list of schedules for automation project. Create 1 jira ticket for each schedule or template identified in the list.  
For each jira ticket, provide jira ticket id, report schedule, short title for the ticket in 8 words, description in 20-30 words without special characters. display jira list output in comma separated format.
"""
